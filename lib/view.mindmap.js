...
assign(Markmap.prototype, {
  ...
  presets: {
    ...
    'oba': assign(assign({}, defaultPreset), {
      nodeHeight: 10,
      renderer: 'basic',
      color: 'mygray',
      nodePadding: 6
  ...
  colors: assign(
    ...
    { mygray: function() {return function() {return '#303030';}}},
    d3.scale
  ),
  init: function(svg, data, options) {
    ...
    var zoom = this.zoom = d3.behavior.zoom()
      .on("zoom", function () {
        this.updateZoom(d3.event.sourceEvent.type, d3.event.translate, d3.event.scale);
       }.bind(this));
    ...
    this.updateZoom("coucou", state.zoomTranslate, state.zoomScale);
  ...
  updateZoom: function (typeEvent, translate, scale) {
    var state = this.state;
    //console.log("typeEvent=" + typeEvent + "TR=" + translate + "SCALE=" + scale);
    //state.zoomScale = 1;
    zoomAttributes = parseAttributes(new String(this.svg.attr("transform")));
    //console.log(zoomAttributes);
    if(typeEvent == "wheel") {
      if (zoomAttributes.scale) {
        oldscale = zoomAttributes.scale.shift();
        //console.log("asked", scale, "oldscale ", oldscale);
        factoir = (scale - oldscale) * 0.5;
        if (scale < oldscale) {
          newscale = parseFloat(oldscale) - 0.025;
        } else {
          newscale = parseFloat(oldscale) + 0.025;
        }
  ...
  update: function(source) {
    ...
    if (state.autoFit) {
      ...
      this.updateZoom("update", translate, scale);
    }
